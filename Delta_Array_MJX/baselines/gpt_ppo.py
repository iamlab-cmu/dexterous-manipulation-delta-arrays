...
class GPT(nn.Module):
    def __init__(self, state_dim, model_dim, action_dim, num_heads, max_agents, dim_ff, pos_embedding, dropout, n_layers, mha, critic=False, masked=True, gauss=False, SA=False):
        ...
        self.gauss = True
        if self.critic:
            self.fwd_fn = self.forward_critic
            self.final_layer = FinalLayer(model_dim, 1, critic=True)
        else:
            self.fwd_fn = self.forward_actor
    ...
    def forward_actor(self, state, actions, pos, idx=None):
    ...
    def forward_critic(self, state, actions, pos, idx=None):
        state_enc = self.state_enc(state)
        pos_embed = self.pos_embedding(pos).squeeze(2)
                                            
        for layer in self.decoder_layers:
            state_enc = layer(state_enc, pos_embed)
            
        state_enc = self.final_layer(state_enc, pos_embed)
        return state_enc 
...
class Transformer(nn.Module):
    ...
    def forward(self, state, act, pos):
        ...
        logp_pi = gauss_dist.log_prob(act)
        # actions = torch.tanh(actions) * self.act_limit
        entropy = gauss_dist.entropy()
        
        val = self.decoder_critic(state, actions, pos)
        return val, logp_pi.view(-1, self.action_dim), entropy.mean()
    ...
    def get_actions(self, state,  pos, deterministic=False):
        ...
        mu, std = self.decoder_actor(state, actions, pos)
        val = self.decoder_critic(state, actions, pos).squeeze(-1)
        if deterministic:
            return torch.tanh(mu) * self.act_limit, None, val
        
        gauss_dist = torch.distributions.Normal(mu, std)
        actions = gauss_dist.rsample()
        logp_pi = gauss_dist.log_prob(actions)
        return torch.tanh(actions) * self.act_limit, logp_pi, val
    
    def get_val(self, state, pos):
        # squeeze is a stopgap solution lmao. Change it later.
        state = state.unsqueeze(0)
        pos = pos.unsqueeze(0)
        bs, n_agents, _ = state.size()
        actions = torch.zeros((bs, n_agents, self.action_dim)).to(self.device)
        return self.decoder_critic(state, actions, pos)