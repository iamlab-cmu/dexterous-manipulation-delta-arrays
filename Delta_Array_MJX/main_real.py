...
def start_capture_thread(current_traj, lock, rl_device, trad, plane_size, save_vid, vid_name=None):
    ...
    capture_thread = multiprocessing.Process(target=delta_array_real.capture_and_convert, args=(child_conn, current_traj, lock, rl_device, trad, plane_size, save_vid, vid_name), daemon=True)
...
if __name__ == "__main__":
    ...
    lock = threading.Lock()
    stop_event, cap_thread = delta_array_real.start_capture_thread(current_traj, lock, config['rl_device'], config['traditional'], env.plane_size, save_vid=config['save_vid'], vid_name=vid_name)
    time.sleep(5)
    ...
    # next_pose[2] += np.pi/2
    env.soft_reset(init_pose, next_pose)
    ...
    while len(current_traj) > 0:
        ...
        env.move_robots(env.active_idxs, actions, delta_array_real.LOW_Z, from_NN)
