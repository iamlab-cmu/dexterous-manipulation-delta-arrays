...
class DeltaArrayRB(DeltaArrayBase):
    ...
    def set_init_and_goal_pose(self, long_horizon=False):
        ...
        self.rot = R.from_euler('xyz', (np.pi/2, 0, self.yaw))
        self.goal_bd_pts = self.set_body_pose_and_get_bd_pts(self.tx, self.rot)
...
class DeltaArrayRope(DeltaArrayBase):
    ...
    def compute_reward(self, actions, inference):
        ...
        ep_reward = delta/self.args['reward_scale']
        ...
        if not inference:
            if self.compensate_for_actions and self.parsimony_bonus:
                ...
                ep_reward -= 20 * (np.sum(actions[:, 2]<0)/self.n_idxs)
        return (init_dist, final_dist), ep_reward
