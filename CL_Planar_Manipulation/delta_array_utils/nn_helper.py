...
class NNHelper:
    def __init__(self):
        ...
        self.img_size = np.array((1440, 2560))
        self.plane_size = np.array([(35, 5),(230.25, -362.25)])

    def get_nn_robots_with_graph(self, boundary_pts, num_clusters=16):
    ...
    def get_nn_robots_without_graph(self, boundary_pts):
        # kmeans = KMeans(n_clusters=16, random_state=0).fit(boundary_pts)
        # self.cluster_centers = np.flip(np.sort(kmeans.cluster_centers_))
        # state_space = {i:tuple(self.cluster_centers[i]) for i in range(len(self.cluster_centers))}
        # state_space_inv = dict((v, k) for k, v in state_space.items())
        hull = ConvexHull(boundary_pts)
        self.cluster_centers = boundary_pts[hull.vertices]
        A, b = hull.equations[:, :-1], hull.equations[:, -1:]
        idxs = set()
        ...
        idxs = np.array(list(idxs))
        neighbors_pix = self.robot_positions[idxs[:,0], idxs[:,1]]
        idxs = np.flip(idxs, axis=1)
        neighbors_cm = self.robot_positions[idxs[:,0], idxs[:,1]]
        print("NN Idxs: ", idxs)
        # neighbors_pix = neighbors_cm.copy()
        # neighbors_pix[:,1] = -1*neighbors_pix[:,1]*self.img_size[1]/(self.plane_size[1][1]-self.plane_size[0][1])-self.plane_size[0][1]
        # neighbors_pix[:,0] = -1*neighbors_pix[:,0]*self.img_size[0]/(self.plane_size[1][0]-self.plane_size[0][0])-self.plane_size[0][0]
