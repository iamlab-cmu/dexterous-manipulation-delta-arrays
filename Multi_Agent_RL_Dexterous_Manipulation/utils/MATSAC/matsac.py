...
class MATAC:
    def __init__(self, env_dixt, hp_dict, logger_kwargs=dict(), train_or_test="train"):
        ...
        self.tf = Transformer(self.obs_dim, self.act_dim, self.act_limit, self.hp_dict["model_dim"], self.hp_dict["num_heads"], self.hp_dict["dim_ff"], self.hp_dict["n_layers_dict"], self.hp_dict["dropout"], self.device, self.hp_dict["delta_array_size"])
