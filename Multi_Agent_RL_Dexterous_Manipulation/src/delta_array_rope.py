...
class DeltaArraySim:
    ...
    def set_block_pose(self, env_idx, goal=False):
        """ 
            Set the rope pose to a random configuration
...
################################################################################################################################################################################
    ...
    def yaw_error(self, pose1, pose2):
        ...
        q_diff = R.from_quat([pose2[3:]]).inv() * R.from_quat([pose1[3:]])
    ...
    def bc_step(self, env_idx, agent):
        ...
        obj_name_enc = torch.as_tensor(self.obj_name_encoder.transform(obj_names))
